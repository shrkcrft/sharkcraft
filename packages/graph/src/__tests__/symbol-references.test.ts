...
describe('extract — bindings + identifier refs', () => {
  test('captures named/default imports and call/non-call references', () => {
    ...
    try {
      ...
      const bindings = ex.importBindings.map((b) => `${b.localName}=${b.specifier}#${b.importedName}(${b.isDefault ? 'default' : 'named'})`).sort();
      expect(bindings).toContain('O=./m#other(named)');
      expect(bindings).toContain('alpha=./m#default(default)');
      // Type-only imports are skipped.
      expect(bindings.find((s) => s.includes('OnlyType'))).toBeUndefined();
