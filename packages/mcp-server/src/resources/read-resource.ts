...
import { parseResourceUri } from './resource-uris.ts';
...
export function readResource(
  ...
  uri: string,
): IReadResourceResult | IReadResourceError {
  ...
  if (parsed.kind === 'knowledge' && parsed.id) {
    ...
    if (!entry) return { ok: false, error: `No knowledge entry with id "${parsed.id}".` };
    return {
      ok: true,
      contents: [
        {
  ...
  if (parsed.kind === 'template' && parsed.id) {
    ...
    if (!template) return { ok: false, error: `No template with id "${parsed.id}".` };
    const safe = {
      id: template.id,
      name: template.name,
  ...
  if (parsed.kind === 'docs' && parsed.path) {
    ...
    try {
      ...
      return {
        ok: false,
        error: `Failed to read doc ${parsed.path}: ${(e as Error).message}`,
      };
    }
    return {
      ok: true,
