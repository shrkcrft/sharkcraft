...
export const listKnowledgeTool: IToolDefinition = {
  ...
  async handler(input, ctx) {
    const types = (Array.isArray(input.types) ? (input.types as string[]) : []).concat(
      typeof input.type === 'string' ? [input.type] : [],
    );
    const scope = (input.scope as string[] | undefined) ?? [];
    const tags = (input.tags as string[] | undefined) ?? [];
    const appliesWhen = (input.appliesWhen as string[] | undefined) ?? [];
    ...
    if (tags.length) entries = entries.filter((e) => tags.every((t) => e.tags.includes(t)));
