...
export const createGenerationPlanTool: IToolDefinition = {
  ...
  async handler(input, ctx) {
    ...
    if (!template) return { isError: true, text: `No template with id "${templateId}"` };
    ...
    if (!result.ok) return { isError: true, text: result.error.message };
