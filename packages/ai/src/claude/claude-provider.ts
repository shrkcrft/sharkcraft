  ...
  async send(request: IAiRequest): Promise<Result<IAiResponse, AppError>> {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return err(
          new AppErrorImpl(ERROR_CODES.IO_ERROR, `Claude API ${res.status}: ${text.slice(0, 500)}`),
        );
      }
      ...
      return err(
        new AppErrorImpl(ERROR_CODES.IO_ERROR, `Failed to call Claude: ${(e as Error).message}`, {
          cause: e,
        }),
      );
    }
